<template>
    <view class="resetPassword flex flex-wrap justify-start">
        <slot name="header">
            <h3>重置密码</h3>
        </slot>
        <view class="input_wrapper">
            <view class="inputItem" :class="{'focus':isFocus.password1,'password_error':errorFlag.password1}">
                <view class="inputContent">
                    ...
                    <input type="password" @focus="focus('password1')" @blur="blur('password1')" ref="password1" placeholder="请输入密码" @input="inputPwd">
                </view>
            </view>
            <view class="inputItem" :class="{'focus':isFocus.password2,'password_error':errorFlag.password2,'password_dif':errorFlag.password2_dif}">
                <view class="inputContent">
                    <img :src=icons.password_icon2 alt="" class="icon">
                    <input type="password" @focus="focus('password2')" @blur="blur('password2')" ref="password2" placeholder="再次输入密码" @input="validatePassword">
                </view>
            </view>
        </view>
    ...
    export default {
        ...
        methods:{
			inputPwd: function(e) {
				...
				this.input_info.password1 = password1;
                this.errorFlag.password1 = password1 === "" || password1 === null || password1 === undefined;
				this.changeBtnState();
			},
			//验证两次输入密码是否一致
			validatePassword: function(e){
			    let password1 = this.input_info.password1;
			    let password2 = e.detail.value;
				this.input_info.password2 = password2;
                this.errorFlag.password2 = password2 === "" || password2 === null || password2 === undefined;
			    this.errorFlag.password2_dif = password1 !== password2;
				this.changeBtnState();
			},
			// 决定按钮的禁用状态
			changeBtnState: function() {
                if (this.input_info.password1 && this.input_info.password2 && !this.errorFlag.password1 && !this.errorFlag.password2 && !this.errorFlag.password2_dif) {
