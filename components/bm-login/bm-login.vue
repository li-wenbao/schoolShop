<template>
    <view class="login flex flex-wrap">
        <slot name="header">
            <h2>登录</h2>
        </slot>
        <view class="input_wrapper">
            <view class="inputItem" :class="{'focus':isFocus.username,'username_error':errorFlag.username}">
                <img :src=icons.username_icon alt="" class="icon">
                <view class="inputContent">
                    <input type="text" @focus="focus('username')" @blur="blur('username')" ref="username" placeholder="请输入用户名" @input="inputUsername" autofocus>
                </view>
            </view>
            <view class="inputItem" :class="{'focus':isFocus.password,'password_error':errorFlag.password}">
                <img :src=icons.password_icon alt="" class="icon" @click="switchVisible">
                <view class="inputContent">
                    <input v-if="!visible" type="password" @focus="focus('password')" @blur="blur('password')" ref="password" placeholder="请输入密码" @input="inputPassword">
					<input v-else type="text" @focus="focus('password')" @blur="blur('password')" ref="password" placeholder="请输入密码" @input="inputPassword">
                </view>
            </view>
        </view>

        <view class="auto_quickLogin">
            <view v-if="mConfigs.register && mConfigs.quickLogin" class="quickLogin" @click="$emit('phoneLogin')">
                手机快捷登录
            </view>
        </view>
        ...
        <view v-if="mConfigs.register && !mConfigs.forgetPassword">
            <button @click="$emit('register')" class="registerBtn">注册</button>
        </view>

        <view v-if="!mConfigs.register && mConfigs.quickLogin">
            <button @click="$emit('phoneLogin')" class="phoneLogin">{{mConfigs.phone_login_text}}</button>
        </view>

        <view class="forgetPwd_register_protocol" v-if="mConfigs.forgetPwd_register_protocol" :class="{'footer':(mConfigs.forgetPassword||mConfigs.register||mConfigs.protocol)&&!mConfigs.otherLoginWays}" v-show="hideFooter">
            <view class="forgetPwd_register" :class="{'space_between':mConfigs.forgetPassword&&mConfigs.register}">
                <view class="forget_password" v-if="mConfigs.forgetPassword" @click="$emit('forgetPassword')">
                    忘记密码
                    <span v-if="!mConfigs.register">?</span>
                </view>
                <view class="separator" v-if="mConfigs.forgetPassword && mConfigs.register">
                    |
                </view>
                <view class="register" v-if="mConfigs.forgetPassword && mConfigs.register" @click="$emit('register')">
                    用户注册
                </view>
            </view>
            <view class="protocol" v-if="mConfigs.protocol && mConfigs.forgetPassword">
        ...
        <view class="otherLoginWays" v-if="mConfigs.otherLoginWays" v-show="hideFooter">
            ...
            <view class="icons">
                <slot name="otherLoginWays_icons">
                    <!-- <img class="icon" src="../../static/images/icon_QQ.png" @click="$emit('qqLogin')">
                    <img class="icon" src="../../static/images/icon_weixin.png" @click="$emit('weixinLogin')">
                    <img class="icon" src="../../static/images/icon_weibo.png" @click="$emit('weiboLogin')"> -->
                </slot>
            </view>
        </view>
        <view class="protocol" v-if="mConfigs.protocol && (!mConfigs.forgetPwd_register_protocol || !mConfigs.forgetPassword)" v-show="hideFooter">
    ...
    export default {
        ...
        methods:{
			...
			inputUsername(e) {
				...
				this.input_info.username = username;
				this.errorFlag.username = username === "" || username === null || username === undefined;
				this.changeBtnState();
			},
			// 用户输入密码
			inputPassword(e) {
				let password = e.detail.value;
				this.input_info.password = password;
				this.errorFlag.password = password === "" || password === null || password === undefined;
				this.changeBtnState();
			},
			// 决定按钮的禁用状态
			changeBtnState() {
			    if (this.input_info.username && this.input_info.password && !this.errorFlag.username && !this.errorFlag.password) {
            ...
            switchVisible(){
                ...
                //输入框为空则显示错误信息
                let password_value = this.input_info.password;
        ...
        .auto_quickLogin{
            ...
            .autoLogin_wrapper{
                .autoLogin{
                    ...
                    width: 12px;
                    height: 12px;
                    background-image: url("../../static/unchosen.png");
                ...
                input[type="checkbox"]:checked + .autoLogin{
                    background-image: url("../../static/chosen.png");
                }
                span{
                    margin-left: .2rem;
                }
